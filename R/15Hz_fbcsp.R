class1 <- read.csv('feature vectors/FBCSP/15hz/feature_agg_target_only.csv')
data <- class1[,3:8]
class2 <- read.csv('feature vectors/FBCSP/15hz/feature_agg_non_target_only.csv')
...
ggplot(data = data.plot) +
  geom_point(aes(x =V1 , y = V2, color = label), size =3.5) + labs(x = "PC1", y = "PC2", title = "15Hz Features (FBCSP)")
